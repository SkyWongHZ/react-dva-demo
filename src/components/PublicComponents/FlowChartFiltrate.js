        ...
        <Form className="t-ML15 lest" layout="inline" key="myForm">
          { items &&
            items.map((item, index) => {
             ...
             /*    console.log('initialValue',item.initialValue) */
                return (<FormItem {...itemLayout} label={item.label} key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                      initialValue: item.initialValue ? item.initialValue:(item.options.length ? typeof item.options[0] !== 'string' ? Util.numToString(item.options[0].value) : item.options[0] : ""),
                    })(
                      <Select
                        size={config.size}
                        showSearch
                      ...
                      >
                        {
                          item.options.map(option => {
                            ...
                            return (
                             /*  <Select.Option key={typeof option === 'string' ? option : option.value} value={String(option.value)}>
                                {item.initialValue ? item.flag:(typeof option === 'string' ? option : option.text)}
                              </Select.Option>
                               */
                               /*  <Select.Option key={typeof option === 'string' ? option : option.value} value={typeof option === 'string' ? option : option.value}>
                                  {typeof option === 'string' ? option : option.text}
                                </Select.Option> */
                              /*   <Select.Option key={typeof option === 'string' ? option : option.value} value={item.flag?String(option.value):(typeof option === 'string' ? option : option.value)}>
                                  {item.flag?item.flag:(typeof option === 'string' ? option : option.text)}
                                </Select.Option> */

                              <Select.Option key={typeof option === 'string' ? option : option.value}  value={(typeof option === 'string' ? option : String(option.value))}>
                                {typeof option === 'string' ? option : option.text}
                              </Select.Option>
                            )
                          })
