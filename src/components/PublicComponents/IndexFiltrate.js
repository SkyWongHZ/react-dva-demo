...
class Filtrate extends Component {
  ...
  render() {
    ...
    // })
    const filtrateContainer = this.props.className !== undefined ? 'filtrateContainer ' + this.props.className : 'filtrateContainer'
    return (
      <div className={filtrateContainer} style={this.props.style}>
        <Form layout="inline" key="myForm">
          {items &&
            items.map((item, index) => {
              if (item.type === 'rangePicker') {
                let disabled = item.disabledDate ? disabledDate : null;
                const itemLayout = item.itemLayout || rangerPickerItemLayout
                return (<FormItem {...itemLayout} className="customFormItem" label={item.label}
                  key={index} style={{ width: item.width || '334px' }}>
                  {
                ...
                return (<FormItem {...formItemLayout} className="customFormItem" label={item.label}
                  key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                ...
                return (<FormItem {...itemLayout} label={item.label} key={index} className="customFormItem">
                  {
                    getFieldDecorator(item.paramName, {
                      initialValue: t.props.defaultValue?t.props.defaultValue:item.options.length ? item.options[0].value: "",
                    })
                    (
                      <Select
                        size={config.size}
                      ...
                      >
                        {
                          item.options.map(option => {
                            return (
                              <Select.Option key={typeof option === 'string' ? option : option.value} value={typeof option === 'string' ? option : option.value} className='indexSelect'>
          ...
          <FormItem className="wp-btn-row" style={{ width: 70 }}>
            {this.props.searchBtnShow &&
              ...
              ghost={true} className="customButton" size="small"
              >
                查询
            </Button>
            }
        ...
        <span className="borderSmallLeftTop"></span>
        <span className="borderSmallRightTop"></span>
        <span className="borderSmallLeftBottom"></span>
        <span className="borderSmallRightBottom"></span>
      </div>
    )
  }
}
