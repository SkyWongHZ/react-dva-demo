...
import { Modal, Form, Input, Button, Select, DatePicker, Row, Col,Checkbox, notification, Upload, Icon,message,InputNumber} from 'antd';
import moment from 'moment';
import MyTable from '../PublicComponents/MyTable';
import config from '../../config';
import Util from '../../utils/Util';
import '../PublicComponents/components-css.less';
  ...
  componentDidMount (){
   /*  if(this.props.defaultData){
    ...
    if (this.props.defaultData) {
      ...
      console.log('this.props.modalItems',this.props.modalItems)
      this.props.modalItems&&this.props.modalItems.map((data, index) => {
                ...
                if (item.sub) {
                            ...
                            item.sub.map((item2, index2) => {
                              if (item2.type === 'input') {
                                  return (
                                    ...
                                    <FormItem {...layout||item2.formItemLayout} label={item2.label}
                                              key={index2}>
                                      {
                                        getFieldDecorator(item2.paramName, {
                                          ...
                                          rules: item2.rules,
                                            ...
                                            <Input key={index2} disabled={item2.disabled}   {...this.props.modalDisabled}/>
                                        )
                                      }
                                  ...
                                  <FormItem {...layout} label={item2.label}
                                            key={index2}>
                                    {
                                      getFieldDecorator(item2.paramName, {
                                        ...
                                        <InputNumber min={0} max={200} step={0.1}  precision={6}
                                  ...
                                  key={index2}>
                                  {
                                    getFieldDecorator(item2.paramName, {
                                      ...
                                      // initialValue: ''
                                          ...
                                          <RangePicker size="large" disabled={item2.disabled}   {...this.props.modalDisabled}> 
                                      </RangePicker>
                                      )
                                  }
                                 </FormItem>
                                ...
                                return (
                                  ...
                                  key={index2}>
                                  {
                                    ...
                                    (
                                      <CheckboxGroup disabled={item2.disabled} options={item2.options} onChange={this.onChange}   {...this.props.modalDisabled}></CheckboxGroup>
                                    )
                                  }
                                </FormItem>
                                </Col>
                  ...
                  return (
                      <div key={index} className="t-MB10">
                       ...
                       {/*  {item.custom} */}
                        <Upload {...this.props.imgProps}>
                          <img className="add-last-day" src={this.props.handleImgPath?this.props.handleImgPath:addImg} alt=""/>
                        </Upload>
                        <Button onClick={()=>this.props.handleUpload('s')} key="submit" type="primary" size="large" className="save">上传图片</Button>
