...
import '../PublicComponents/PublicComponentsStyle.less';
import MyIcon from '../PublicComponents/MyIcon';
      ...
      <div className={`yk-components-filtrate ${this.props.className && this.props.className}`}>
        <Form className="t-ML15 lest" layout="inline" key="myForm">
            ...
            items.map((item, index) => {
              ...
              if (item.type === 'input') {
                ...
                return (<FormItem {...itemLayout} label={item.label} key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                      initialValue: item.options.length ? typeof item.options[0] !== 'string' ? Util.numToString(item.options[0].value) : item.options[0] : "",
                    })(
                      <Select
                        size={config.size}
                        showSearch
                ...
                return (<FormItem {...rangerPickerItemLayout} className="range-picker" label={item.label}
                  ...
                  {
                    getFieldDecorator(item.paramName, {
                      initialValue: [this.changeDate(moment()), this.changeDate(moment().add(1, 'd'))]
                    })(
                      <RangePicker
                        size={config.size}
                                  ...
                                  key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                      ...
                      <MonthPicker
                        ...
                        format="YYYY"
                        allowClear={item.allowClear ? false : true}
                      />
                    )
                  }
                </FormItem>)
          ...
          {this.props.extraType &&
            ...
            <Tabs activeKey={this.props.historyValue} onChange={this.props.extraTypeToggle} className="yk-filtrate-tabs">
            ...
            extraBtn.map((btn, btnIndex) => {
              return (
                <FormItem key={btnIndex} className='yk-btn-row'>
                  ...
                  >
                   {this.props.iconType&&
                    <MyIcon className="t-ML10 t-MR4" style={{ fontSize: '12px', color: '#3996FF ' }} type="icon-chaxun"  onClick={t.extraBtnClick.bind(t, btnIndex, btn.funName)}/>}
                    {btn.name}
                  </Button>
                </FormItem>
              )
        ...
        {/* 关闭按钮 */}
        {t.props.closeTitle &&
          <MyIcon onClick={this.props.closeTitleFn} className="t-ML10 t-MR4 close-circle"  style={{ fontSize: '18px', color: 'rgb(255, 41, 56)', float: 'right', cursor: 'pointer' }} type="icon-guanbi" />
        }
      </div>
    )
  }
