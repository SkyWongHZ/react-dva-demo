...
class AlarmInformation extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            columns: [
    ...
    //   let params = this.filtrate.getFieldsValue();
    //   let startTime = this.changeDate(params.year[0]).format('x')
    //   let endTime = this.changeDateEnd(params.year[1]).format('x')
    // 初始化时间
    // changeDate = (date) => {
    //     const currentDate = moment(date);
    //     currentDate.set({ 'hour': 0, 'minute': 0, 'second': 0, 'millisecond': 0 });
